...
class FPLJ {
   public:
    ...
    PS::F64vec force;
    PS::F64 potential;
    PS::F64vec getPos() const { return q; }
    void setPos( PS::F64vec nq ) { q = nq; }
...
class EPLJ {
   public:
    ...
    PS::F64vec q;
    PS::F64vec getPos() const { return q; }
    void setPos( const PS::F64vec &nq ) { q = nq; }
    PS::F64 getRSearch() const { return CUTOFF_LENGTH; }
...
class CalcForceLJ {
   public:
    void operator()( const EPLJ *epi, const PS::S32 ni, const EPLJ *epj, const PS::S32 nj, ForceLJ *force ) {
...
PS::F64 energy( const Tpsys &system ) {
    PS::F64 e = 0.0;
...
void drift( Tpsys &system, const PS::F64 dt ) {
    const PS::S32 n = system.getNumberOfParticleLocal();
    for( PS::S32 i = 0; i < n; i++ ) { system[ i ].q += system[ i ].p * dt; }
}
//------------------------------------------------------------------------
template <class Tpsys>
void kick( Tpsys &system, const PS::F64 dt ) {
    const PS::S32 n = system.getNumberOfParticleLocal();
    for( PS::S32 i = 0; i < n; i++ ) { system[ i ].p += system[ i ].force * dt; }
}
//------------------------------------------------------------------------
template <class C>
void add_atom( PS::F64vec q, PS::F64vec c, PS::F64 r, PS::F64 xv, C &atoms ) {
    Atom a;
    a.q = q;
...
//------------------------------------------------------------------------
template <class C>
void put_ball( PS::F64vec c, PS::F64 r, PS::F64 xv, double density, C &atoms ) {
...
//------------------------------------------------------------------------
template <class C>
void export_cdview( int rank, int procs, C &system ) {
    static int index = 0;
    char filename[ 256 ];
    sprintf( filename, "output/conf%03d.cdv", index );
    if( 0 == rank ) {
        std::ofstream ofs( filename );
        if( ofs.bad() ) throw std::runtime_error( "Cannot open file" );
...
//------------------------------------------------------------------------
template <class C>
int make_conf( double density, PS::F64 L, int rank, C &lj_system ) {
